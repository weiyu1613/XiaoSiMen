...
document.addEventListener('DOMContentLoaded', function() {
  console.log('📚 TeachAny 道德与法治课件系统加载完成');
...
function checkPreQuiz(element, isCorrect) {
  ...
  if (isCorrect) {
    ...
    if (window._comboCount && window._comboCount >= 3) document.dispatchEvent(new CustomEvent('mascot:combo', {detail: {count: window._comboCount}}));
...
/* P0-16: Level up event dispatch for mascot */
function showLevelUp(newLevel) {
    if (typeof _origShowLevelUp === 'function') _origShowLevelUp(newLevel);
    document.dispatchEvent(new CustomEvent('mascot:levelup', {detail: {level: newLevel}}));
}
